...
public interface IArticleMapper extends BaseMapper<Article> {
    ...
    @Select("select * FROM article where article_tags=#{tag} order by publish_date")
    List<Article> findListByTag(@Param("tag") String tag, IPage<Article> articleIPage);

    @Select("select * FROM article where article_categories=#{category} order by publish_date")
    List<Article> findListByCategory(@Param("category") String category, IPage<Article> articleIPage);

    @Select("select DISTINCT article_tags FROM article ")
    List<String> findTags();

    @Select("select DISTINCT article_categories FROM article ")
    List<String> findCategorys();
}
